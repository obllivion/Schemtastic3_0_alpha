...
void XNOR_component::PaintComp(QPainter &p)
{
    if(!ref.isNull())
    {
        if(selektovan==true){
...
void XNOR_component::setParent(QWidget *parent){
    tabelica=new QTableWidget(parent);
}

void XNOR_component::setTabelicu(int brUlaza){
   ...
   // int br[brUlaza];
    ...
    tabelica->setWindowTitle("XNOR");

}

void XNOR_component::mojaFja(){
    int zbir=0;
