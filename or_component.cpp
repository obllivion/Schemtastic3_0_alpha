...
void OR_component::PaintComp(QPainter &p){
    if(!ref.isNull())
    {
        if(selektovan==true){
              p.setPen(Qt::red);
...
void OR_component::setParent(QWidget *parent){
    tabelica=new QTableWidget(parent);
}

void OR_component::setTabelicu(int brUlaza){
   ...
   // int br[brUlaza];
    ...
    tabelica->setWindowTitle("OR");
}

void OR_component::izracunajOpet(int ul, int ko)
{
    this->ulazi[ko]=ul;
    this->mojaFja();
}

void OR_component::mojaFja()
{
    int zbir=0;
